...
from clippt import PresentationApp, Slide, Presentation
from clippt.slides import load_slide, FuncSlide, Slide, MarkdownSlide, ShellSlide
...
def create_presentation():
    def md(source, *, title=None): return MarkdownSlide(source=source, title=title)

    def sh(command, **kwargs): return ShellSlide(source=command, cwd=CWD, **kwargs)

    def load(source, **kwargs): return load_slide(CWD / source, **kwargs)
    ...
    return Presentation(
        ...
        slides = [
            load_slide(CWD / s) if isinstance(s, str) else s
            for s in slides
        ]
    )
